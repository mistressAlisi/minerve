...
def paginator_json_response(_obj):
    """
    ...
    @rtype: JSONResponse
    """
    return JsonResponse({"res": "ok", "paginator": {"total_pages": round(_obj.count() / 20), "current_page": 1, "total_records": _obj.count()}})
...
def paginator_paginate_object(_obj,page=1,page_size=20):
    ...
    total_pages = round(_obj.count() / page_size)
    if total_pages >= 2:
